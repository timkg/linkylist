(function() {
	...
	exports.compileModel = function() {
		...
		EmbedlyExtractModel.getExtractForUrls = function(listOfUrls, callback) {
			...
			embedlyservice.getExtractForUrls(listOfUrls, function(embedlyApiResponse) {
				exports.saveDocuments(embedlyApiResponse, callback);
			});
		};
	...
	exports.assignEmbedToLink = function(embed, callback) {
		LinkModel = require('./Link').compileModel();
	...
	exports.promiseToSaveDocument = function(json) {
		...
		mongoose.models.EmbedlyExtractModel
			.findOne({original_url: json.original_url})
			.exec(function(err, embed) {
				if (!embed) {
					mongoose.models.EmbedlyExtractModel.create(json, function(err, embed) {
						if (err || !embed) { deferred.reject(new Error(err)); return false; }
						exports.assignEmbedToLink(embed, function(err, embed) {
	...
	exports.saveDocuments = function(json, callback) {
		if (typeof json === 'string') { json = JSON.parse(json); }
		if (!Array.isArray(json)) { json = [json]; }

		var allOperationsAsPromises = [];
		json.forEach(function(embedValue) {
			var embedPromise = exports.promiseToSaveDocument(embedValue).fail(function(error) {
				console.warn(error);
			});
