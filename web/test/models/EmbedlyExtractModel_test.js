(function() {
	...
	exports.test_compilesModelFromSchema = function(test) {
		test.ok(EmbedlyExtractModel, 'compiled EmbedlyExtract model');
		test.done();
	};

	exports.test_createsEmbedlyInstances = function(test) {
		var emb = new EmbedlyExtractModel(
	...
	exports.test_savesInstancesToDb = function(test) {
		...
		emb.save(function(err, result) {
			if( err ) { throw err; }
			test.equals(result.provider_url, "http://www.ua.pt",'created an embedly instance');
			test.done();
		});
	};

	exports.test_findsEmbedliesByUrl = function(test) {
		EmbedlyExtractModel.find({
			provider_url: "http://www.ua.pt"
		}, function(err, results) {
			if( err ) { throw err; }
			test.equals(results.length, 1, 'found an embed object');
			test.done();
		});
	};
	...
	exports.test_savesResponseFromServiceRequestWithLinkRelation = function(test) {
		...
		EmbedlyExtractModel.saveDocuments(json, function() {
			LinkModel
				.findOne({
					url: "http://www.ua.pt"
				})
				...
				.exec(function(err, result) {
					...
					test.equals(result._embedlyExtract.original_url, "http://www.ua.pt", 'found associated EmbedExtract');
					test.done();
				}
			);
		});
